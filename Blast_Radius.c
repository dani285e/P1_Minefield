...
int main(void) {
    ...
    blast_radius(ACTIVE_MINERS, true_false, mine_count, mines); // Kalder blast_radius funktionen
    return 0;
}


Blast blast_radius(int active_miners, int *true_false, int mine_count, Mines * mines) {
    Blast blast[mine_count * 9]; // Minecount er 9, da arrayet omkring bomben er 3 x 3
    if (active_miners > 1) {
    ...
    while(*true_false == 1) { // Imens der er mere end 1 deminer
        for (int i = 0; i < mine_count; i++) {   // Den tæller, hvor mange miner der er. Den starter ved 0 og tæller op
            for (int j = 0; j < 9; j++) { // Den tæller, hvor mange felter i arrayet omrking minen, som skal kategoriseres som radius
                blast[i] = add_koordinat(mines[i].y, mines[i].x, mines[i].mineID);
                blast[blast_index] = add_koordinat(mines[i].y--, mines[i].x, mines[i].mineID); // Under minen
                ...
                blast[blast_index] = add_koordinat(mines[i].y--, mines[i].x++, mines[i].mineID); // Skråt for under minen
                blast_index++;
                blast[blast_index] = add_koordinat(mines[i].y, mines[i].x++, mines[i].mineID); // Til højre for minen
                blast_index++;
                blast[blast_index] = add_koordinat(mines[i].y++, mines[i].x++, mines[i].mineID); // Højre skrå hjørne
                blast_index++;
                blast[blast_index] = add_koordinat(mines[i].y++, mines[i].x, mines[i].mineID); // Over minen
                blast_index++;
                blast[blast_index] = add_koordinat(mines[i].y++, mines[i].x--, mines[i].mineID); // Venstre skrå hjørne
                blast_index++;
                blast[blast_index] = add_koordinat(mines[i].y, mines[i].x--, mines[i].mineID); // Til venstre for minen
                blast_index++;
                blast[blast_index] = add_koordinat(mines[i].y--, mines[i].x--, mines[i].mineID); // Venstre nederste hjørne
            }
        }
    }
...
/* void radius_of_explosion(int n) {
    pow((2 * BLAST + 1), 2);
    return 0;
}     */ // Hvis man vil lave det med flere miner, så skal denne funktion bruges